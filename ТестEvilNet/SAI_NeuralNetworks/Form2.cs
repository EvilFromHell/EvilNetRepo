...
namespace SAI_NeuralNetworks
{
    ...
    public partial class Form2 : Form
    {
        ...
        {
            var lst = Layers.ToList();
            lst.Insert(0, SizeX);
            NET = new EvilNeuralNet(lst);
            path = "";
        ...
        private void открытьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(path))
            {
                try
                {
                    NET = EvilNeuralNet.Deserialize(path);
                }
                finally
                { }
                ...
                txtLogs.AppendText("Число входов: " + Convert.ToString(NET.FirstLayer.Count) + "\r\n");
                txtLogs.AppendText("Число выходов: " + Convert.ToString(NET.LastLayer.Count) + "\r\n");
                txtLogs.AppendText("Число скрытых слоёв: " + Convert.ToString(NET.Layers.Count-2) + "\r\n");

                for (int i = 1; i < NET.Layers.Count - 1; i++)
                {
                    txtLogs.AppendText("Нейронов в " + Convert.ToString(i) + " скрытом слое: "
                                                        + Convert.ToString(NET.Layers[i].Count) + "\r\n");
                }
            }
            else 
            {
        ...
        private void сохранитьКакToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    EvilNeuralNet.Serialize(NET,path);
                }
                finally
                { }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            double[] X = new double[NET.FirstLayer.Count];
            double[] Y = new double[NET.LastLayer.Count];
            String[] currFile;
            ...
            while (kErr > kErrNorm)
            {
                ...
                for (currPos = 0; currPos < txtLernFiles.Lines.Count() - 1; currPos++)
                {
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < NET.FirstLayer.Count; i++)
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            Y = NET.CalculateResult(X).ToArray();
            //NET.NetOUT(X, out Y);
